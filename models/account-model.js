...
/* *****************************
* Return account password using account_id
* ***************************** */
async function getPasswordById (account_id) {
  try {
    const result = await pool.query(
      'SELECT account_password FROM account WHERE account_id = $1',
      [account_id])
    return result.rows[0].account_password
  } catch (error) {
    return new Error("No matching email found")
  }
}
...
* *************************** */
async function editAccount(account_id, account_firstname, account_lastname, account_email, account_password){
  try {
